          ...
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ delay: 0.1, duration: 0.6 }}>
            ...
            <ul className="space-y-3">
              {quickLinks.map((link: any) => (
                <li key={link.label}><Link href={link.href} className="text-gray-medium hover:text-teal transition-colors duration-300 text-sm flex items-center gap-2 group"><span className="w-1.5 h-1.5 bg-teal rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300" />{link.label}</Link></li>
              ))}
            </ul>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ delay: 0.2, duration: 0.6 }}>
            ...
            <div className="space-y-4">
              <div className="flex items-start gap-3">
                ...
                <div><p className="text-white text-sm font-medium">Address</p><p className="text-gray-medium text-sm whitespace-pre-line">{address}</p></div>
              </div>
              <div className="flex items-start gap-3">
                ...
                <div><p className="text-white text-sm font-medium">Email</p><a href={`mailto:${email}`} className="text-gray-medium text-sm hover:text-teal transition-colors">{email}</a></div>
              </div>
              <div className="flex items-start gap-3">
                <svg className="w-5 h-5 text-teal flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                <div><p className="text-white text-sm font-medium">Phone</p><a href={`tel:${phone}`} className="text-gray-medium text-sm hover:text-teal transition-colors">{phone}</a></div>
              </div>
            </div>
          </motion.div>
        </div>
      ...
      <div className="border-t border-white/5">
        <div className="container-custom py-6">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <p className="text-gray-medium text-sm text-center sm:text-left">&copy; {new Date().getFullYear()} {schoolName}. All Rights Reserved.</p>
            ...
            <p className="text-gray-medium text-sm text-center sm:text-right">Developed by <a href={devUrl} target="_blank" rel="noopener noreferrer" className="text-teal font-semibold hover:text-white transition-colors duration-300">{devName}</a></p>
          </div>
        </div>
      </div>
    </footer>
