                      ...
                      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -30 }} className="bg-white dark:bg-navy-light rounded-2xl shadow-xl border border-teal/20 dark:border-teal/10 p-8">
                        ...
                        <div className="mb-6"><label className="block text-sm font-medium text-navy dark:text-white mb-3">Select Payment Method</label>
                          <div className="grid grid-cols-2 gap-3">
                            {[
                              { id: "mtn" as PaymentMethod, label: "MTN Mobile Money", color: "border-yellow-400", ussd: "*165#" },
                              { id: "airtel" as PaymentMethod, label: "Airtel Money", color: "border-red-400", ussd: "*185#" },
                            ].map((m) => (
                              <button key={m.id} onClick={() => setPaymentMethod(m.id)} className={`p-4 rounded-xl border-2 transition-all text-center ${paymentMethod === m.id ? `${m.color} bg-teal/5 dark:bg-teal/10` : "border-gray-200 dark:border-navy-light hover:border-gray-300"}`}>
                                <span className="text-lg font-bold text-navy dark:text-white block mb-1">{m.ussd}</span>
                  ...
                  <AnimatePresence>
                    {showInstructions && paymentMethod && (
                      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -30 }} className="bg-white dark:bg-navy-light rounded-2xl shadow-xl border border-teal/20 dark:border-teal/10 p-8">
                          ...
                          <ol className="space-y-3 text-sm">
                            {steps.map((s, i) => (
                              <li key={i} className="flex gap-3">
                                <span className="w-6 h-6 bg-teal text-white rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0">{s.step}</span>
                                <span className="text-charcoal dark:text-gray-300">
                                  {s.text}{" "}
                                  {s.code ? (
                                    s.step === 6 && paymentMethod === "mtn" ? (
                                      <strong className="text-teal">{code}</strong>
                                    ) : s.step === 5 && paymentMethod === "airtel" ? (
                                      <strong className="text-teal">{code}</strong>
                                    ) : s.step === 7 && paymentMethod === "mtn" ? (
                                      <strong className="text-teal">{paymentAmount.toLocaleString()} UGX</strong>
                                    ) : s.step === 6 && paymentMethod === "airtel" ? (
                                      <strong className="text-teal">{paymentAmount.toLocaleString()} UGX</strong>
                                    ) : (
                                      <strong className="text-teal">{s.code}</strong>
                                    )
                                  ) : (
                                    s.text.includes("Amount") ? <strong className="text-teal">{paymentAmount.toLocaleString()} UGX</strong> :
                                    s.text.includes("Payment Code") ? <strong className="text-teal">{code}</strong> : null
                                  )}
                                </span>
                              </li>
                            ))}
                        ...
                        <motion.button onClick={handleCheckPaymentStatus} disabled={paymentChecking} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="btn-primary w-full py-4 text-base disabled:opacity-60">
                          {paymentChecking ? <span className="flex items-center justify-center gap-2"><div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" />Checking Payment...</span> : "I've Paid — Check Status"}
                        </motion.button>
